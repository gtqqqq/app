...
<mapper namespace="${daoPackageName}.${tableName}Mapper">
  ...
  ${column.dbcolumnName},</#list>
    ...
    <insert id="insertSelective" parameterType="${poPackageName}.${tableName}PO">
    ...
    <update id="updateByPrimaryKeySelective"
            parameterType="${poPackageName}.${tableName}PO">

        update ${dbtableName}
        <set>
             <#list columns as column>
            <if test="${column.columnName} != null">
                 ${column.dbcolumnName} = ${'#{'?html}${column.columnName},jdbcType=${column.dbcolumnType}},
            </if>
             </#list>
        </set>
             <where>
    ...
    <select id="selectBy${tableName}List" parameterType="${poPackageName}.${tableName}PO"
            resultMap="BaseResultMap">
        SELECT
        <include refid="Base_Column_List"/>
        FROM ${dbtableName}
